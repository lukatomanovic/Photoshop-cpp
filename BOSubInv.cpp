...
BOSubInv::BOSubInv() :Operation("subinv",1), constant_num_(0)
{
}


bool BOSubInv::ValidateAndSetParams(std::string params)
{
	std::regex rx("[ ]*([0-9]+)[ ]*");
	std::smatch result;
...
void BOSubInv::Execute(std::vector<int>& layer, const std::map<std::pair<unsigned, unsigned>, unsigned>& selected_pixels_)
{
	...
	if (selected_pixels_.size()) {
		std::for_each(selected_pixels_.begin(), selected_pixels_.end(), [&layer, w, this](const std::pair<std::pair<unsigned, unsigned>, unsigned>& tek) {
			...
			layer[start_position] = constant_num_ - layer[start_position];
			layer[start_position + 1] = constant_num_ - layer[start_position + 1];
			layer[start_position + 2] = constant_num_ - layer[start_position + 2];
			});
	}
	else {
		int h = layer[layer.size() - 1];
		for_each(layer.begin(), layer.end(), [this](int& color) {color = constant_num_ - color; });
