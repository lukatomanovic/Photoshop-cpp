...
BOAdd::BOAdd():Operation("add",1),constant_num_(0)
{
}


bool BOAdd::ValidateAndSetParams(std::string params)
{
	//std::regex rx("[ ]*([0-9]*)[ ]*");
	//std::smatch result;
	...
	//}
	std::vector<int> &&p=Operation::ParamsParserToIntVect(params);
	if (number_of_params_ != p.size()) {
		throw ParamsError();
		params_set_ = false;
		return false;
	}
...
void BOAdd::Execute(std::vector<int>& layer, const std::map<std::pair<unsigned, unsigned>, unsigned>& selected_pixels_)
{
